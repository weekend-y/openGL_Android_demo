...
void MyGLRender::SetShaderIndex(int shaderIndex) {
    LOGCATE("MyGLRender::SetShaderIndex shaderIndex = %d", shaderIndex);
    m_ShaderIndex = shaderIndex;
    m_IsShaderChanged = true;

}

int MyGLRender::GetShaderIndex() {
    LOGCATE("MyGLRender::GetShaderIndex");
...
float MyGLRender::GetFrameProgress() {
...
void MyGLRender::SetShaderProgramDynamicAttrib(int shaderIndex) {
    float progress = GetFrameProgress();

    LOGCATE("MyGLRender::SetShaderProgramDynamicAttrib progress=%f, m_ShaderIndex=%d",
    ...
    switch (shaderIndex) {
        case COLOR_SHIFT_SHADER_INDEX:
            ...
            LOGCATE("MyGLRender::SetShaderProgramDynamicAttrib transformMatrix.scaleX=%f",
...
void MyGLRender::LoadFilterImageData(int index, NativeImage *pImage) {
    LOGCATE("MyGLRender::LoadFilterImageData pImage = %p, index=%d", pImage->ppPlane[0],
            index);
    ByteFlowLock lock(&m_SynLock);
    NativeImageUtil::FreeNativeImage(&m_ExtRgbaImage);
...
void MyGLRender::LoadFragShaderScript(int shaderIndex, char *pShaderStr, int strLen) {
    LOGCATE("MyGLRender::LoadFragShaderScript pShaderStr = %p, shaderIndex=%d", pShaderStr,
            shaderIndex);
